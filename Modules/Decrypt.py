...
def decrypt_files(decrypt_type: str, disk: str, settings_path: str) -> dict:
    ...
    while True:
        try:
            path = settings_path + 'secrets.txt'
            with open(path, 'rb') as file:
                file_data = file.read()
                break
        except Exception as error:
            logger.error(f'Error with trying to open file secrets.txt! {error}')
